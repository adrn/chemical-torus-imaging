...
$\vec{x}_\ast$ is the na\"ive mean of the training labels $y_i$ for the $k$
nearest training-set neighbors $i$ in the vector space $\vec{x}$:
\begin{equation}
  y_ast \leftarrow \frac{1}{k}\,\sum_{i=1}^k y_i
  \quad .
\end{equation}
...
\begin{equation}
  ...
  \quad ,
\end{equation}
where $a$ is an intercept and $b$ is a slope. The intercept $a$ is the linear
fit interpolated to the position $\vec{x}_\ast$ of the test point... HOGG
