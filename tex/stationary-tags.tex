...
\begin{abstract}\noindent
  The kinematics of stars in galaxies are often modeled in an approximation
  where the gravitational potential is treated as simple and integrable and the
  distribution function as time-invariant.
  Under these conditions, there are a number of standard tools for performing
  dynamical inferences of the underlying mass distribution in these systems
  using observations of stellar kinematics.
  However, contemporary and near-future spectroscopic surveys measure more than
  kinematics for huge samples of individual stars throughout the Milky Way and
  Local Group.
  ...
  stellar labels, such as ages).
  This method exploits the fact that, in the Galaxy, stellar labels vary
  systematically with orbit characteristics (i.e., actions), yet must be
  invariant with respect to orbital phases (i.e., time-dependent conjugate
  angles).
  Thus, the orbital foliation of phase space must coincide with surfaces of
  constant statistical-moments of all stellar label distributions!
  % TODO: the preceding sentence is hella clunky
...
individual stars.
Instead, these methods rely on strong assumptions about the distribution
...
Zhai:2018, Buch:2019}, while the Jeans \textit{equations} are correct for any
collisionless tracer population of stars, implementing Jeans models in practice
requires assuming that the system is in steady-state or equilibrium and requires
specifying an explicit parametrized model for the underlying gravitational
potential \citep[see, e.g.,][for a review of such methods as applied to the
problem of determining the local dark matter density]{Read:2014}.
More general approaches here instead attempt to model the \acronym{DF}
explicitly \citep[e.g.,][]{McMillan:2013, Magorrian:2014, Binney:2014,
...
spheroidal galaxies, standard methods for inferring the underlying mass
distribution from a population of tracers therefore typically assume an
equilibrium (or steady-state) distribution function, (coordinate) separability
of the \acronym{DF}, simple parametrizations of the \acronym{DF} and mass model,
and more.
