% To-dos
% ------
% - is there a word for iso-composition?? isosynth
...
% - Figure 4: abundances for all abundances that we use here
...
% Torus tomography
% Abundance-moment foliation
%   Abundance-moment laminae
% Toroidal abundance blender
%   Toroidal abundance mixer
...
\begin{abstract}\noindent % Hogg strikes again. <-- what a guy
  ...
  measuring properties of the underlying mass distribution in galaxies.
  ...
  %The tags directly reveal the orbit structure.
  We demonstrate the concepts and methods central to this idea using red giant
  stars from the \apogee\ surveys to look at vertical orbit structure in the
  Milky Way disk, and constrain the disk mass (holding the well-measured
  circular velocity fixed).
  We show that the disk mass can be constrained na\"ively at the XXX-percent
  level with this new method using only a single element abundance, thus
  demonstrating the promise of using this ...todo.
  We discuss both classical-statistics and Bayesian forward-modeling approaches
  to inferences of the Galactic mass distribution using these ideas.
...
% Notebook: Figure1.ipynb
\begin{figure}[!t]
  \begin{center}
  \includegraphics[width=\textwidth]{orbits-mean-abun-z-vz}
  \end{center}
  \caption{%
    Left panel: The mean \mgfe\ abundance as a function of cylindrical
    Galactic coordinate height $z$ and vertical velocity $v_z$.
    Averages are taken in $z, v_z$ boxels.
    The stars at lower $z$ and lower $v_z$, that is the stars with
    lower overall $z$ action $J_z$, show lower \mgfe on average.
    Other panels: Same as the left panel, but with two orbits
    ...
    The orbits appear as finite-thickness lines because the orbits are
    finite-volume 3-tori in 6-space projected onto the $z$--$v_z$
    plane.
    APW TODO: Also shown are the locations of a few values of the
    conjugate angle $\theta_z$ in radians.
  \label{fig:zvz-mgfe}
  }
\end{figure}
