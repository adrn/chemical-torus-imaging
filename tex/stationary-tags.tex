...
relatively specifically predicted by the actions:
The method only requires that there are gradients in some moments of the
abundance distribution.
There can be---and there is---a wide range of abundance ratios at every point in
action space.
In this project, all distributions are expected to have (and are observed to
have) substantial dispersion.
...
Like Jeans modeling, Schwarzschild modeling, or distribution function modeling, \methodname\ provides a way to infer the underlying acceleration field traced by [...], for which we only have access to instantaneous measurements of phase-space coordinates.
...
radial kinematics.
Indeed the orbits plotted in \figurename~\ref{fig:zvz-mgfe}
