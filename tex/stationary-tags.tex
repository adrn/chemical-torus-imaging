...
The ``chemical tagging'' insight \cite{Freeman:2002} is that most stars also
preserve their surface element abundances (and other stellar tags) as they
orbit (i.e., over Galactic dynamically-relevant timescales).
...
Now consider how this abundance distribution varies as we consider different
regions in phase space.
In general, the abundance distribution will vary: For example, there are
observed radial and vertical abundance gradients in the Galactic disk
\citep{people}.
...
In this \documentname, we demonstrate the utility of these gradients in the
context of dynamical inferences in the Milky Way, where we can measure
six-dimensional kinematics and element-abundances for individual stars.
For the sake of illustration and simplicity, we only visualize and focus on the
vertical kinematics of stars, but we \emph{do not} assume that the potential or
distribution function is separable.
We emphasize that we could have equivalently focused on radial kinematics (using
the cylindrical radial velocity and position), or considered gradients in all
phase-space coordinate simultaneously: This methodology is generic!
...
This idea was motivated by plots of elemental abundances of stars as a function
of vertical height $z$ and vertical velocity $v_z$ in Galactocentric Cartesian
coordinates, colored by element abundances;
...
no residual dependence of abundances on angles.
In the second---which we will call \emph{generative}---a predictive model
is built, in which the abundances are drawn from a distribution that is
a funciton of actions alone (and not ever angles).
...
APW: Get more specific about forms and math. Call out to figures.

APW: How do we avoid binning; how do we set our uncertainties; how do we
get an answer with an uncertainty?
