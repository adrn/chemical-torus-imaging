...
physicists of the 18th century, who worked out that the gravitational force law
...
is, they used observations of a few tracers (the planets) at many orbital
phases.
...
of stellar streams \citep[e.g.,][]{peoples}, but these are relatively sparse and
their orbits have more degrees of freedom than orbits around a point-mass.
One could argue then that our current task is much harder than Newton and Kepler
had it: We seek to constrain the global distribution of dark matter around a
galaxy---a time-evolving mass distribution with nontrivial shape and radial
distribution---with only a snapshot of the tracer kinematics (and often only a
subset of phase-space dimensions).
The tools we use to constrain the force law with a snapshot of the dynamics are
therefore (necessarily) statistical in nature and do not depend on knowing the
orbits of individual stars.
Instead, these methods, such as ``Jeans modeling'' \citep{Binney:2008},
typically rely instead on assumptions about the stationarity of distributions of
orbital properties, and make use of theories that relate moments of the tracer
distribution function to properties of the force field under these conditions.
\textbf{APW SAYS: say more about Jeans modeling, and maybe quote some examples?
Like Oort? And be very specific about what assumptions are typically made}.
Standard methods for estimating XX therefore rely on a list of assumptions about
the dynamical state and symmetries of the systems (in the case of Jeans
modeling, LIST ASSUMPTIONS THAT GO IN), many of which we know are violated in
the real universe and Galaxy.
However, methods that enable dynamical inferences while relaxing these
assumptions are far more complex and subtle and ... so we think there is still
value in working under bad assumptions.

One sense in which we are at a \emph{huge advantage} over Kepler and Newton is
that we have excellent working model of stars and immense observational and
computational resources.
With these resources we have measured the element abundances of the stellar
photospheres of hundreds of thousands of stars (CITE) and precise positions and
velocities of millions.
In this \documentname\ we are going to demonstrate that these measurements can
be used to illuminate the orbit structure in the Milky Way, even (or especially)
in the dense, nearby parts of the Galactic disk.
Our method, like all other dynamical inferences, will work only under strong
assumptions, so we are still in the business of revealing the orbits indirectly.
\textbf{Comments about the hope that our assumptions are less restrictive, and make use of more information (i.e., stellar tags), so has promise}
However, our approach is novel and there are many regimes in which it will be
more precise than any other method.

A principle concept in our approach is that, in a well-mixed population, stars
are in a kinematic steady state:
As time goes on, stars move along their orbits, but they don't change their
dynamical invariants (by assumption!) and every location in angle (and by
``angle'' we mean the coordinate conjugate to action in action--angle
coordinates) is equally likely, or equally probably populated in any snapshot.
The chemical-tagging insight (CITE) is that the stars also don't change their
surface element abundances as they orbit.
That is, in an integrable galaxy---a galaxy whose gravitational orbits are
associated with three invariant actions---the element abundances and the
dynamical actions have something in common:
...
That's a remarkably informative constraint on the configuration of the Milky Way
in the space of positions (3), velocities (3), and detailed abundances (10 to
30, depending on the spectroscopic survey).
...
That is, their abundances are drawn from some distribution in $D$-dimensional
element-abundance space.
Now consider how this abundance distribution varies as we move around in phase
space.
It will vary in general (there are radial and vertical abundance gradients in
the disk, for example).
Any gradient we observe in these element-abundance distributions with respect to
phase-space coordinates (that is, the six-gradient) must not project onto the
directions of increasing (or decreasing) conjugate angles in phase space.
That is, all gradients with respect to phase-space coordinates of the
element-abundance distribution must be orthogonal to the directions of increase
(or decrease) of the conjugate angles, and lie in the subspace of the directions
of increase (or decrease) of the dynamical actions.
The trajectories of stars in the phase space (the dynamical tori) must lie along
or describe level surfaces in the element-abundance distribution!

In this \documentname, we demonstrate the utility of this
...
\begin{figure}[!tp]
  ...
  \caption{%
    ...
    parameter $M_\disk / M^\star$. The data points show the values of the disk-mass
