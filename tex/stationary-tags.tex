...
midplane and a different solar $z$ velocity.
The top left and top right panels of \figurename~\ref{fig:coeff-mdisk} show
that, at all values of the disk mass parameter \mratio, there is a finite $m=1$
amplitude for both the $\cos$ and $\sin$ terms.
...
\begin{equation}
  f(a_1, b_1, a_2 \,;\, \mratio, z_\odot, v_{z, \odot}) =
    a_1^2 + b_1^2 + a_2^2
\end{equation}
where we ignore the constant term $c_0$ and the amplitude of the $\sin
2\theta_z$ term $b_2$.
...
performed with a bootstrap sample).
...
\figurename~\ref{fig:elem-ellipses=1} shows
...
We assert above that \methodname\ is unlike Jeans modeling, and (in
particular) more stable and less mathematically constrained.
