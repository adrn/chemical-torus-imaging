...
\begin{abstract}\noindent
  ...
  Local Group.
  Here we present a qualitatively new approach for dynamical
...
and stellar tags.
That's out of scope for this \documentname, but suggests possible generalizations
of \methodname\ that account for stellar populations that are incompletely
angle-mixed.
Beyond the Galactic disk in the stellar halo, as with Jeans modeling,
\methodname\ will be biased by the existence of unmixed substructures (streams, shells, and
merger remnants; e.g., \citealt{TODO}).
...
based on a nearest-neighbor interpolation in action space
(see \sectionname~FOO and \appendixname~BAR).
That is a blunt tool; in principle results could improve if we model the abundance
distribution moments as a function of actions.
One kind of option for this modeling would be to use a generative, physical model
for star-formation history, nucleosynthesis, chemical enrichment, and stellar
migration.
Another would be to use a machine-learning method such as a Gaussian process.

In what we present here, we have parameterized the Milky Way mass model with a
very simple, few-component mass model.
Another option would be not to parameterize the mass model, but instead
parameterize the torus foliation of phase space directly.
This foliation could be very general; any foliation of phase space with closed
3-tori is in principle possible, dynamically.
Then the foliation could be interpreted in terms of the force law, and hence the
potential or mass model.
That would be more data-driven approach---and with far fewer assumptions---than what's
been executed here.
...
\section{Re-weighting the $k$ nearest neighbors to account for steep number
density gradients}
\label{app:knn-weights}

\TODO{Oo boy, here we go...}
