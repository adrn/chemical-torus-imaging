...
% - Remove Jeans comment from abstract
...
\begin{abstract}\noindent
  ...
  Local Group.
  Here we present a qualitatively new approach for dynamical inference,
  \methodname, which makes use of kinematic measurements and also element
  abundances (or other stellar labels, e.g., stellar ages).
  ...
  selection function nor velocity dispersion estimates.
...
of stellar streams \citep[e.g.,][]{peoples}, where ensembles of stars almost
...
% TODO: audit that we say the right things about Jeans...
...
statistical moments of the tracer distribution function (\acronym{DF}) to
properties of the force field.
...
sequences have different scale lengths \citep[e.g.,][]{Bovy:2016}, and our toy
...
% TODO: now we also vary scale height!
...
\begin{figure}[!tp]
  ...
  \caption{%
    ...
    the solar velocity $h_{z}$ for each element individually (colored ellipses)
    and the joint constraint (black).
...
\TODO{Comment on how exactly this can bias us with our toy implementation: It can cause variations in the mean abundance anomaly that will be subsumed into our simple parameters and interpreted as incorrect vzsun or zsun or Mdisk. In detail, it will depend on the scale (in angle-space) of the dominant perturbations (if ``large-scale,'' will more impact zsun/vzsun. If ``small-scale,'' will affect mass parameter more). Similar issues with generative versions of the method, but can be accounted for more naturally by explicitly modeling them?}
