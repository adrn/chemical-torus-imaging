...
% - KVJ: still unsure why you don't emphasize not needing 'dynamical
%   equilibrium' as a key assumption you can drop (in abstract and end) .... the
%   requirement that in Jeans the density distribution of your tracer population
%   sits in the mass distribution and you're not just tracing orbits. --- She’s
%   saying that we don’t require *equilibrium* just mixing. I don’t think Jeans
%   requires equilibrium either, but some models/methods do?? It was a statement
%   about global equilibrium vs. locally (in an action sense?) mixed. Jeans
%   equations don't require equlibrium, Jeans models do?
% - We don't have to bake in a simple model of the DF - we're more like
%   Scwarzschild in that sense!
% - Add an algorithm figure or box or table or etc. that gives the
%   stages of calculation from mass-model parameters to angles to objective.

% send it to
% ----------
% - Glenn Van de Ven -- Schwarzschild models of external galaxies
% - Loebman -- we discussed in Aspen
...
\begin{abstract}\noindent
  The dynamics of stars in galaxies are often modeled in an approximation where
  the gravitational potential is treated as simple and integrable and the
  distribution function as time-invariant.
  Under these conditions, ``Jeans models'' and ``Schwarzschild models'' are
  standard tools for measuring properties of the underlying mass distribution
  from observations of stellar kinematics.
  ...
  makes use of kinematic measurements and also element abundances (or other
  stellar labels, e.g., stellar ages).
  The method exploits the fact that stellar labels vary systematically with
  orbit characteristics (such as actions) in the Galaxy, yet must be invariant
  with respect to the (time-variable) conjugate angles.
  Thus, the orbital foliation of phase space must coincide with isoclinics of
  all (mean) stellar labels!
  Both classical-statistics and Bayesian methods can be built on this idea;
  these methods will be both more robust and require fewer assumptions than
  standard tools because they neither require knowledge of the (spatial) survey
  selection function nor depend on velocity dispersion measurements.
  We perform a classical-statistics demonstration with red giant branch stars
  from the \apogee\ surveys:
  we model the vertical orbit structure in the Milky Way disk and use mean
  abundances to constrain the local disk mass, scale height, and the disk--halo
  mass ratio (at fixed local circular velocity).
...
as separability of the distribution function (\acronym{DF}), or the velocity
anisotropy profile, or require simple parametrizations of the \acronym{DF} in
...
preserve their surface element abundances (and other stellar labels) as they
...
\begin{figure}[!tp] % Notebook: figures/Abundance-anomaly-bootstrap.ipynb
  ...
  \caption{%
    ...
    are never used in the analysis, because binning is sinning and all that).
...
\NoCaptionOfAlgo
\begin{algorithm}[tp]
  \caption{\textbf{Procedure for computing the classical-statistics objective function}}
  \SetAlgoLined
  \KwIn{Parameter vector $(\mdisk, \hz, \zsun, \vzsun)$ and bootstrap sample of parent stellar sample}
    \qquad Compute the halo mass at fixed $v_{\rm circ}(R_\odot) = 229~\kms$ \\
    \qquad Compute Galactocentric positions and velocities for each star\\
    \qquad Compute St\"ackel potential focal length parameter for each star \\
    \qquad Compute actions and angles for all stars (St\"ackel fudge) \\
    \qquad Compute action-local mean abundance deviation for each star (\equationname~\ref{eq:mean-abun-dev})\\
    \qquad Use linear least-squares to compute the optimal Fourier parameters (\equationname~\ref{eq:fouriermodel})\\
    \qquad Compute the objective function (\equationname~\ref{eq:objective}).
   \label{alg:objective}
\end{algorithm}
...
\begin{figure}[!tp]
  \begin{mdframed}
    \color{captiongray}
  \begin{center}
  \includegraphics[width=0.8\textwidth]{other-error-ellipses.pdf}
  \end{center}
  \caption{%
