...
% - Move "Bayesian" formulation to discussion
%   - In this context, we also measure the gradients themselves, which are
%     interesting too!
...
% - Das/Sanders stuff: assumes that abundances depend on actions or vice versa
%   - Us: there are properties of abundance distribution that ...

% - Make plots for a red clump-only dataset to compare to RGB
...
% TODO: we might want to add something like ": A novel method for dynamical mass
% model measurements" or something. We want to tell people who don't care or
% know what "orbital foliations" are that this is cool and novel!
...
The final parent sample contains \nstars\ RGB stars with high-quality \apogee\ and
\gaia\ data.
...
\abunratio{X}{Y} on the vertical angle, we define an ``mean abundance-ratio
